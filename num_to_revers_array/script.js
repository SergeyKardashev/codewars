...
function digitize_11(n) {
    return Array.from(String(n)).map((i) => parseInt(i)).reverse();
}
...
function digitize_09_long(n) {
    const str = String(n);
    const arr = Array.from(str, (letter) => { return Number(letter) });
...
function digitize_08_long(n) {
    ...
    const arrOfNum = arrOfStrings.map((symbol) => { return Number(symbol) })
